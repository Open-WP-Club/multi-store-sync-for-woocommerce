...
class ProductExclusionFilterTest extends WC_Multi_Store_TestCase
{
    ...
    /**
     * Test is_category_excluded helper
     */
    public function test_is_category_excluded(): void
    {
        $store_config = [
            'exclude_categories' => [10, 20, 30],
        ];

        $this->assertTrue(
            WC_Multi_Store_Product_Exclusion_Filter::is_category_excluded(20, $store_config)
        );

        $this->assertFalse(
            WC_Multi_Store_Product_Exclusion_Filter::is_category_excluded(50, $store_config)
        );
    }

    /**
     * Test is_category_excluded matches via a parent category (ancestor expansion),
     * consistent with should_exclude_by_ids()
     */
    public function test_is_category_excluded_via_parent_category(): void
    {
        $GLOBALS['wc_mss_test_get_ancestors'] = [30 => [20]];
        ...
        try {
            $this->assertTrue(
                WC_Multi_Store_Product_Exclusion_Filter::is_category_excluded(30, $store_config)
            );
        } finally {
            $GLOBALS['wc_mss_test_get_ancestors'] = null;
        }
    }

    /**
     * Test is_category_excluded with empty config
     */
    public function test_is_category_excluded_empty_config(): void
    {
        $store_config = [];

        $this->assertFalse(
            WC_Multi_Store_Product_Exclusion_Filter::is_category_excluded(10, $store_config)
        );
    }

    /**
     * Test is_tag_excluded helper
     */
    public function test_is_tag_excluded(): void
    {
        $store_config = [
            'exclude_tags' => [100, 200, 300],
        ];

        $this->assertTrue(
            WC_Multi_Store_Product_Exclusion_Filter::is_tag_excluded(200, $store_config)
        );

        $this->assertFalse(
            WC_Multi_Store_Product_Exclusion_Filter::is_tag_excluded(500, $store_config)
        );
    }

    /**
     * Test is_tag_excluded with empty config
     */
    public function test_is_tag_excluded_empty_config(): void
    {
        $store_config = [];

        $this->assertFalse(
            WC_Multi_Store_Product_Exclusion_Filter::is_tag_excluded(100, $store_config)
        );
    }
