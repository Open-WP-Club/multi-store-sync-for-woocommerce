...
class RemoteProductManagerTest extends WC_Multi_Store_TestCase
{
    ...
    public function test_delete_removes_remote_product(): void
    {
        $api = $this->createMockApi();

        $api->shouldReceive('get_products')->with('DEL-SKU', 'sku')->andReturn([
            ['id' => 42, 'sku' => 'DEL-SKU'],
        ]);
    ...
    public function test_delete_skips_when_not_found_on_remote(): void
    {
        $api = $this->createMockApi();

        $api->shouldReceive('get_products')->andReturn([]);

        $result = $this->manager->delete($api, 'MISSING-SKU');

        $this->assertTrue($result['success']);
        $this->assertEquals('skipped', $result['action']);
    }

    public function test_delete_fails_with_empty_sku(): void
    {
        $api = $this->createMockApi();

        $result = $this->manager->delete($api, '');

        $this->assertFalse($result['success']);
        $this->assertStringContainsString('no SKU', $result['message']);
    }

    public function test_delete_handles_api_search_error(): void
    {
        $api = $this->createMockApi();

        $api->shouldReceive('get_products')->andReturn(
            new \WP_Error('api_error', 'Search failed')
        );

        $result = $this->manager->delete($api, 'ERR-SKU');
    ...
    public function test_restore_publishes_trashed_product(): void
    {
        $api = $this->createMockApi();
        ...
        $api->shouldReceive('update_product')->with(42, ['status' => 'publish'])->once()->andReturn(['id' => 42]);

        $result = $this->manager->restore($api, 'RESTORE-SKU');
    ...
    public function test_restore_skips_already_published(): void
    {
        $api = $this->createMockApi();
        ...
        $result = $this->manager->restore($api, 'ACTIVE-SKU');

        $this->assertTrue($result['success']);
        $this->assertEquals('skipped', $result['action']);
    }

    public function test_restore_skips_when_not_found(): void
    {
        $api = $this->createMockApi();

        $api->shouldReceive('get_products')->andReturn([]);

        $result = $this->manager->restore($api, 'GONE-SKU');

        $this->assertTrue($result['success']);
        $this->assertEquals('skipped', $result['action']);
    }

    public function test_restore_fails_with_empty_value(): void
    {
        $api = $this->createMockApi();

        $result = $this->manager->restore($api, '');
    ...
    public function test_update_status_changes_remote_status(): void
    {
        $api = $this->createMockApi();
        ...
        $api->shouldReceive('update_product')->with(42, ['status' => 'draft'])->once()->andReturn(['id' => 42]);

        $result = $this->manager->update_status($api, 'STATUS-SKU', 'draft');

        $this->assertTrue($result['success']);
        $this->assertEquals(42, $result['remote_id']);
        $this->assertEquals('draft', $result['new_status']);
    }

    public function test_update_status_fails_when_not_found(): void
    {
        $api = $this->createMockApi();

        $api->shouldReceive('get_products')->andReturn([]);

        $result = $this->manager->update_status($api, 'MISSING-SKU', 'draft');

        $this->assertFalse($result['success']);
        $this->assertTrue($result['product_not_found']);
    }

    public function test_update_status_fails_with_empty_sku(): void
    {
        $api = $this->createMockApi();

        $result = $this->manager->update_status($api, '', 'draft');
